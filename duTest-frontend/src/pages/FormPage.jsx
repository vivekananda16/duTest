...
function FormPage() {
  ...
  useEffect(() => {
    if (isEditMode) loadTestCase();
  }, [id]);

  const loadTestCase = async () => {
    const response = await getTestCaseById(projectId, id);
    setFormData({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isEditMode) {
      await updateTestCase(projectId, id, formData);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-sm w-full max-w-2xl p-8">
        <div className="mb-8">
          <button onClick={() => navigate(`/projects/${projectId}/testcases`)}
          ...
          <p className="text-gray-500 mt-1 text-sm">
            {isEditMode ? "Update existing test case details." : "Add a new test case to your project."}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="text" name="testCaseID" value={formData.testCaseID}
              onChange={handleChange} placeholder="Example: TC001" required className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Test Case Name</label>
            <input type="text" name="name" value={formData.name}
              onChange={handleChange} placeholder="Example: Login Test" required className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Description</label>
            <textarea name="description" value={formData.description}
              onChange={handleChange} placeholder="Write test case description..." rows="4" className={inputClass} />
          </div>
          ...
          <div className="flex gap-3 pt-4">
            <button type="button" onClick={() => navigate(`/projects/${projectId}/testcases`)}
